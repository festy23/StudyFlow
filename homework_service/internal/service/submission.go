...
type SubmissionServiceInterface interface {
	...
	ListSubmissionsByAssignment(ctx context.Context, assignmentID string) ([]*domain.Submission, error)
}
...
func (s *submissionService) ListSubmissionsByAssignment(ctx context.Context, assignmentID string) ([]*domain.Submission, error) {
	return s.submissionRepo.ListByFilter(ctx, domain.SubmissionFilter{AssignmentID: assignmentID})
}
